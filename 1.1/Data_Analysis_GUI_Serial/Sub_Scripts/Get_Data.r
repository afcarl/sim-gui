...
print(variable)
		...
		if(agent_number > 1)
		{
			...
			if(method == "mean" && weight != "No")
			{
				...
				rm(W)
			}else if(method == "sd_percent"){

				A<-apply(Z,2,mean,na.rm=TRUE)
				B<-apply(Z,2,sd,na.rm=TRUE)

				
				temp_transformation <- B/A
		
				temp_transformation[is.infinite(temp_transformation)]<-NA
